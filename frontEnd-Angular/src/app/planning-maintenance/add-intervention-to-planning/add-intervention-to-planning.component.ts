...
@Component({
  selector: 'app-add-intervention-to-planning',
  templateUrl: './add-intervention-to-planning.component.html',
  styleUrl: './add-intervention-to-planning.component.css'
})
export class AddInterventionToPlanningComponent implements OnInit {
  public profil: KeycloakProfile | null = null;
  constructor(private ser: PlanningServiceService,private ref:MatDialogRef<AddInterventionToPlanningComponent>,
    ...
    ref.disableClose = true; this.addForm = this.fb.group({
      ...
      heureDebut: ['', Validators.required], equipement: [this.equipement1, Validators.required],
        ...
        this.ser.checkIntervention(this.intervention.name).subscribe((exists) => {
          if (exists) {
            ...
            if (this.intervention.type === 'Preventive') {
              ...
              this.ser.addIntervention(this.intervention, idTech, this.data.eq.id, this.equipement.id, idRespo).subscribe((data) => {
                console.log(data);
                this.closePopup();
